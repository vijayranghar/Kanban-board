...
export default function Column({ title, tickets, groupingType, available=false }) {
  ...
  const iconUrl = ICON[title] ? (
    ...
    <span className="user-initial">{getInitials(title).toUpperCase()}
      {available ?<span className="status"/> : <span className="status offline"/>}
    </span>
  );
  return (
